...
var devLight = new Light(znode, 19)
...
znode.on('attribute_change', function (event) {
  if (devLight.endpoint_id === event.endpoint_id) {
    /* Light status changed */
    console.log('light status =', devLight.onoff_get_value())
  }
})
...
if (options.mode === 'ezmode') {
  ...
  devLight.ezmode_commissioning_target_start()
} else if (options.mode === 'default') {
  if (status === 'joined_network' || status === 'joining_network') {
    /* Network is already configured - Get onoff light status */
    console.log('- current light status =', devLight.onoff_get_value())
  } else {
