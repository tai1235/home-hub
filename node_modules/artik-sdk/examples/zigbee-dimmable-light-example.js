...
var devDimmableLight = new DimmableLight(znode, 20)
...
znode.on('attribute_change', function (event) {
  if (devDimmableLight.endpoint_id === event.endpoint_id) {
    /* Light status changed */
    console.log('light status =', devDimmableLight.onoff_get_value())
    console.log('light level =', devDimmableLight.level_control_get_value())
  }
})
...
if (options.mode === 'ezmode') {
  ...
  devDimmableLight.ezmode_commissioning_target_start()
} else if (options.mode === 'default') {
  if (status === 'joined_network' || status === 'joining_network') {
    /* Network is already configured - Get dimmable light status */
    console.log('- current light status =', devDimmableLight.onoff_get_value())
    console.log('- current light level =', devDimmableLight.level_control_get_value())
  } else {
